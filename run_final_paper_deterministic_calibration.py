...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--work-limit", type=float, default=None)
    return p.parse_args()
...
def main() -> None:
    ...
    s.validate()
    work_limit = float(s.final_planner_work_limit if args.work_limit is None else args.work_limit)
    wall_seconds = max(
    ...
    train_weeks = base.build_bundles(
        scoped, train_starts, cfg=cfg, candidate_pools=context,
        eligibility_maps=base.build_eligibility_maps(hist, cfg), offset=0,
    )
    ...
    dm = {w.position: arrays.booked[arrays.week_slices[w.position]] for w in subset}

    t0 = time.perf_counter()
    plans = science.deterministic_eval_solve_batch(
        subset, dm, s, work_limit=work_limit, wall_seconds=wall_seconds,
        gap=s.final_planner_gap, label="training_deterministic_calibration",
    )
    ...
    report = {
        ...
        "phi_accounting_max_tolerance": protocol.PHI_ACCOUNTING_MAX_TOL,
