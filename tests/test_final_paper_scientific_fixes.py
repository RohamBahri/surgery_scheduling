...
def test_scientific_settings_freeze_new_counts_capacity_and_box() -> None:
    ...
    s.validate()
    assert science.EXPECTED_CLEANED_CASES == 32269
    assert science.EXPECTED_SCOPED_CASES == 30260
    assert s.expected_train_cases == 20951
    assert s.expected_holdout_cases == 6700
    assert s.coefficient_bound == 100.0
    assert science.PRIMARY_ROSTER == "median_count_template"
    assert science.REQUIRED_CAPACITY_SENSITIVITY == "regular_template"
    assert science.EXPECTED_FEATURES == 107
    assert science.MAX_VALID_ROOM_DURATION_MINUTES == 1440.0
    assert s.final_planner_work_limit > 0
...
def test_common_cost_lambda_is_shared_across_cost_scale_methods() -> None:
    final.install_final_adapter()
    science.apply_scientific_fixes()
    s = science.ScientificFinalSettings(data="dummy.xlsx", artifact_root="artifacts/test")
    a = base.Arrays(
        X=sparse.csr_matrix(np.array([[1.0, 0.0], [1.0, 1.0]])),
        booked=np.array([100.0, 120.0]),
        actual=np.array([130.0, 100.0]),
        error=np.array([30.0, -20.0]),
        week_ids=np.array([0, 0]),
        case_ids=np.array([1, 2]),
        week_slices={0: np.array([0, 1])},
    )
    lam = science.common_cost_lambda(a, s)
    ra = science.ScientificFixedSpec(
        "RA", a, np.array([1.0, 1.0]), np.array([1.0, 1.0]), None, lam, s
    )
    raf = science.ScientificFixedSpec(
        "RA_FULL", a, np.full(2, s.overtime), np.full(2, s.idle), None, lam, s
    )
    os = science.ScientificFixedSpec(
        "OS", a, np.full(2, s.overtime), np.full(2, s.idle), None, lam, s
    )
    assert ra.lam == raf.lam == os.lam == lam


def test_implementable_oracle_respects_behavioral_bounds_and_floor() -> None:
    final.install_final_adapter()
    science.apply_scientific_fixes()
    s = science.ScientificFinalSettings(data="dummy.xlsx", artifact_root="artifacts/test")
    a = base.Arrays(
        X=sparse.csr_matrix(np.ones((3, 1))),
        booked=np.array([14.0, 100.0, 100.0]),
        actual=np.array([1.0, 200.0, 90.0]),
        error=np.array([-13.0, 100.0, -10.0]),
        week_ids=np.array([0, 0, 0]),
        case_ids=np.array([1, 2, 3]),
        week_slices={0: np.array([0, 1, 2])},
    )
    planning, corr = science.implementable_oracle_planning(a, s)
    assert np.all(planning > 0)
    assert corr[0] >= -s.alpha * min(s.h, 13.0) - 1e-9
    assert corr[1] <= s.alpha * s.h + 1e-9
    assert corr[2] == -10.0
